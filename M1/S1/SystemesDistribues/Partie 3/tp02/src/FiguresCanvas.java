 ...
 */
public class FiguresCanvas extends JPanel implements MouseMotionListener, MouseListener, KeyListener, Scribble
{
    /**
    ...
    private ScribbleServer scribbleServer;
    private int id, ctNotify;

    /**
     * Constructor
     */
    public FiguresCanvas(String host, int port, String objName) throws RemoteException, NotBoundException
    {
        ...
        this.scribbleServer = (ScribbleServer)registry.lookup(objName);
        ctNotify = 0;
        notifyServer();
    }
    ...
    public void setColor(Color color)
    {
        ...
        if (this.selectedFigure != null) {
            ...
            try {
                this.scribbleServer.updateFigure(this.selectedFigure, id);
                notifyServer();
    ...
    public void mouseReleased(MouseEvent event)
    {
        ...
        if (dragging) { // a figure is currently drawing or moving
            if (!moveCursor && !resizeCursor) {
                ...
                try {
                    scribbleServer.createFigure(this.selectedFigure, id);
                    notifyServer();
                } catch(RemoteException e) {
                    e.printStackTrace();
                }
                ...
                try {
                    scribbleServer.updateFigure(this.selectedFigure, id);
                    notifyServer();
                } catch(RemoteException e) {
                    e.printStackTrace();
                }
