// tube.c
//
// exemple de communication entre deux processus pere fils
...
int main ( int argc , char **argv ) {
  ...
  if ((pid=fork())== 0) {
    ...
    while(read(tube[0],&c,1) >0)  {
      // affiche le caratere sur la sortie standard 
      write(1," ",1);
      write(1,&c,1);
    }
    printf("fin de lecture du tube\n");
    // ferme le tube
    close(tube[0]);
    // sortie sans erreur
    exit(0);
    ...
    while(read(0,&c,1) >0) {
     ...
     printf("fin de lecture sur l'entree standard\n");
