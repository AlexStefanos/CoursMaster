...
t_edge *prim(float **adjacencyWeight, int order) {
    ...
    while(indexTree < (order-1)) {
        ...
        for(x = 0; x < order; x++) {
            if(marks[x]) {
                for(y = 0; y < order; y++) {
                    if(adjacencyWeight[x][y] && !marks[y] && (adjacency[x][y] < min)) {
        ...
        marks[yMin] = 1;
        tree[indexTree].top1 = xEdgeWeightMin;
        tree[indiceTree].top2 = yMin;
        tree[indiceTree].weight = min;
        indiceTree++;
    }
    return(tree);
}

void displayGraph(t_edge *tree, int order) {
    int i, j;
    printf("------------------------------Affichage du Graphe------------------------------");
    for(i = 0; i < order; i++) {
        for(j = 0; j < order; j++)
            printf("%.1f", adjacency[i][j]);
        printf("\n");
    }
}
...
void displayTree(t_edge *tree, int nbEdges) {
    ...
    for(i = 0; i < nbTree; i++)
        printf("Arête %d :\n\tSommet n°1 : %d\n\tSommet n°2 : %d\n\tPoids : %.1f\n", (i+1), (tree[y].top1 + 1), (tree[i].top2 + 1), tree[i].weight);
    printf("\n");
}
...
int main() {
    ...
    float **adjacencyWeight = loadGraph(order);
    displayGraph(adjacencyWeight, order);
