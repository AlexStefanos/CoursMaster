...
int burn_around(int x, int y, int N, int M, char map[N][M]) {
  ...
  if (y != 0) {
    if (map[y-1][x] == '.')
        map[y-1][x] = 'A';
    else if (map[y-1][x] == 'S' || map[y-1][x] == 'D')
        return(1);
  }
  if (x != 0) {
    if (map[y][x-1] == '.')
        map[y][x-1] = 'A';
    else if (map[y][x-1] == 'S' || map[y][x-1] == 'D')
        return(1);
  }
  if (y != N-1) {
    if (map[y+1][x] == '.')
...
int can_move_dir(int x, int y, char dir, int N, int M, char map[N][M]) {
  if (dir == 'T')
      ...
      return y != N-1 && map[y+1][x] == '.';
  else if (dir == 'L')
      return x != 0 && map[y][x-1] == '.';
  else if (dir == 'R')
      return x != M-1 && map[y][x+1] == '.';
  else
      return(0);
}
    ...
    if (nb_move > 0) {
      ...
      if (nb_move == 1) {
        ...
        if (delta_x > 0 && left)
            ...
            map[y][x+1] = 'D';
        else if (delta_y < 0 && bottom)
            map[y+1][x] = 'D';
        else if (delta_y > 0 && top)
            map[y-1][x] = 'D';
        else if (top)
            map[y-1][x] = 'D';
        else if (bottom)
