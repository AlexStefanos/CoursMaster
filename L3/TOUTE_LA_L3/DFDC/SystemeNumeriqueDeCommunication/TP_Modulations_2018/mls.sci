...
select n								//assign taps which will yeild a maximum
...
for i = (2^n)-1:-1:1
   ...
   xorbit = modulo(abuff(tap1)+abuff(tap2),2);		//feedback bit
   
   if taps==4
      xorbit2 = modulo(abuff(tap3)+abuff(tap4),2);//4 taps = 3 xor gates & 2 levels of logic
      xorbit = modulo(xorbit+xorbit2,2);				//second logic level
   end
