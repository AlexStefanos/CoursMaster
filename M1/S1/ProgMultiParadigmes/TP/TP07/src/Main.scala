class Sudoku(startConfig_ : Array[Array[Int]]) {
  ...
  def solver(): Array[Array[Int]] = {
    ...
    def isDone: Boolean = {
      ...
      for(i <- 0 until nbSide; j <- 0 until nbSide) {
        if(workGrid(i)(j) == 0) {
          result = false
        }
      }
  ...
  override def toString : String = {
    ...
    for(i <- 0 to 8) {
      result += s" $i\n\n"
      for(j <- 0 to 8) {
        val value : Int = startConfig_(i)(j)
        result += s" $value "
      }
      result += "\n"
    }
...
object Main {
  def main(args: Array[String]) : Unit = {
    ...
    val sudo : Sudoku = new Sudoku(grid)
    val departChrono = System.currentTimeMillis()
    val result : Array[Array[Int]] = sudo.solver()
    result.foreach(e => println(e.mkString("(", ", ", ")")))
